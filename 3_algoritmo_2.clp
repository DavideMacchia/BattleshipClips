...
(defrule k-top-fire-one-cells-south (declare (salience 31))
  (status (step ?s)(currently running))
  (d-cell (x ?x) (y ?y) (content top))
  (moves (fires ?fm&:(> ?fm 0)))
	(not (d-cell (x =(+ ?x 1)) (y ?y)))
...
(defrule k-bot-fire-one-cells-north (declare (salience 31))
  (status (step ?s)(currently running))
  (d-cell (x ?x) (y ?y) (content bot))
  (moves (fires ?fm&:(> ?fm 0)))
	(not (d-cell (x =(- ?x 1)) (y ?y)))
...
(defrule k-left-fire-one-cells-east (declare (salience 31))
  (status (step ?s)(currently running))
  (d-cell (x ?x) (y ?y) (content left))
  (moves (fires ?fm&:(> ?fm 0)))
	(not (d-cell (x ?x) (y =(+ ?y 1))))
...
(defrule k-right-fire-one-cells-ovest (declare (salience 31))
  (status (step ?s)(currently running))
  (d-cell (x ?x) (y ?y) (content right))
  (moves (fires ?fm&:(> ?fm 0)))
	(not (d-cell (x ?x) (y =(- ?y 1))))
...
(defrule guess-to-high-prob (declare (salience 15))
  ...
  ;;seleziona la casella con probabilità più alta tra tutte
	;aggiunto il vincolo che la prob deve essere più alta di una certa soglia S=0.4
  ?p <- (p-cell (x ?x) (y ?y) (prob ?prob1&:(>= ?prob1 0.25)))
...
;non c'è il controllo sui fire e i guess perchè potrebbe finire prima di aver usato tutti i guess, la priorità bassa fa in modo che la regola venga attivata all'ultimo
